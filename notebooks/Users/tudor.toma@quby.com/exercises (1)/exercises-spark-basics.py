...
# MAGIC 
# MAGIC *Remember that for this course the default location will be hdfs (the Hadoop Distributed File System).*
# MAGIC 
# MAGIC #### Exercise 4
# MAGIC 
# MAGIC Create a new RDD called `mywords` from the **local** file `'file://{data}/words'.format(data=data)`
...
import numpy as np
numberlist = np.arange(1,10001,1)
numberlist[:10]
mynumbers = sc.parallelize(numberlist,3)
mynumbers.count()
mynumbers.take(10)
...
# MAGIC %md #### Exercise 7
# MAGIC 
# MAGIC Collect all data from the `mynumbers` RDD into a local variable called local_lengths and print the contents of this variable.
# MAGIC 
# MAGIC 
...
mylengths = mywords.map(lambda x: len(x))
mylengths.take(10)
...
filteredwords = mywords.filter(lambda x: x[0].lower()=='e')
...
print(lettercombinations.take(10))
letters = lettercombinations.flatMap(lambda x: x).distinct()
